...
class PortfolioApp {
  ...
  initLoading() {
    // DISABLED FOR INSTANT LOADING - No more loading class or delays
  ...
  delayLandingAnimations() {
    ...
    const heroElements = document.querySelectorAll(
      ".hero-title, .hero-subtitle, .hero-description, .hero-actions, .hero-image"
    );
  ...
  initTheme() {
    ...
    if (savedTheme) {
      ...
      this.isDarkTheme = !window.matchMedia("(prefers-color-scheme: light)")
        .matches;
    }
  ...
  applyTheme() {
    ...
    if (this.isDarkTheme) {
      ...
      if (profileImage) {
        this.switchProfileImage(
          profileImage,
          "./assets/ele/profile_pic.png",
          "Tirth Patel - Developer Illustration (Dark Theme)",
          "brightness(0.7) contrast(1.1) hue-rotate(0) saturate(1)"
        );
      ...
      if (profileImage) {
        ...
        this.switchProfileImage(
          profileImage,
          "./assets/ele/profile2.jpg",
          "Tirth Patel - Developer Illustration (Light Theme)",
          "none"
        );
      }
    }
  ...
  animateChildElements(parent) {
    ...
    const children = parent.querySelectorAll(
      ".tech-badge, .stat, .contact-card, .bento-card"
    );
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            window.addEventListener("scroll", () =>
              this.updateParallaxElement(entry.target)
            );
          }
        });
      },
    ...
    this.observeProjectCards = () => {
      setTimeout(() => {
        const projectCards = document.querySelectorAll(
          ".project-card:not(.animation-completed)"
        );
  ...
  animateCounter(element, finalValue) {
    ...
    const timer = setInterval(() => {
      ...
      if (currentValue >= finalValue) {
        element.textContent =
          finalValue + (element.textContent.includes("+") ? "+" : "");
        clearInterval(timer);
      } else {
        element.textContent =
  ...
  init3DCardEffects() {
    ...
    projectCards.forEach((card) => {
      ...
      const handleMouseMove = (e) => {
        ...
        const maxDistance =
          Math.sqrt(cardWidth * cardWidth + cardHeight * cardHeight) / 2;
  ...
  async loadProjects() {
    try {
      ...
      const response = await fetch(
        `./assets/config/projects.json?v=${timestamp}`
      );
      ...
      this.projects = [
        {
          ...
          description:
            "AI-powered legal assistant project with 3D avatar. Designed an AI-powered legal assistant integrating a 3D avatar and LLM for real-time legal resolution. Combined NLP, legal data processing, and immersive UI to enhance user experience and accessibility in legal tech.",
          technologies: [
            "Python",
            "NLP",
            "LLM",
          ...
          category: "ai",
          image: "./assets/projects/legal-chatbot.png",
        ...
        },
        {
          ...
          description:
            "A cross-platform app that helps students track income & expenses with smart insights. Built using React Native + Firebase for real-time data sync. Designed a clean, user-friendly interface tailored for student needs. Implemented features like expense categorization and monthly analytics.",
          technologies: [
            "React Native",
            "Firebase",
            "JavaScript",
          ...
          category: "web",
          image: "./assets/projects/altuno-finance.png",
      ...
      this.categories = [
        ...
        { id: "ai", name: "AI/ML", icon: "fa-solid fa-brain", visible: true },
        {
          id: "automation",
          name: "Automation",
  ...
  createProjectCard(project, index = 0) {
    ...
    card.innerHTML = `
      ...
      project.title
    }" class="project-image parallax-slow" loading="lazy" 
                 onerror="this.src='https://via.placeholder.com/400x200/1a1a2e/6366f1?text=${encodeURIComponent(
                   project.title
                 )}'">
    ...
    handleContactForm(e) {
        ...
        fetch('./send_email.php', {
            ...
            .catch((error) => {
                ...
                this.showNotification('❌ Failed to send message. Please try emailing me directly at aubalavignesh1010@gmail.com', 'error');
            })
            .finally(() => {
    ...
    cards.forEach((card, index) => {
      ...
      newCard.addEventListener("mouseleave", () => {
        newCard.style.transition =
          "transform all 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275)";
      ...
      newCard.addEventListener("mousemove", (e) => {
        ...
        const maxDistance =
          Math.sqrt(
            cardRect.width * cardRect.width + cardRect.height * cardRect.height
          ) / 2;
        ...
        if (Math.random() < 0.01) {
          // Log only 1% of the time to avoid spam
          console.log(
            `Card ${index}: rotateX=${rotateX.toFixed(
              1
            )}, rotateY=${rotateY.toFixed(1)}, scale=${scale.toFixed(2)}`
          );
